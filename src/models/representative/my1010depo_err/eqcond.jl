...
"""
function eqcond(m::myModel1010depo_err)
    ...
    # Spread return on capital - riskfree rate
    Γ0[eq[:eq_spr], endo[:spr_t]]     = 1.
    Γ0[eq[:eq_spr], endo[:ERtil_k_t]] = -1.
    Γ0[eq[:eq_spr], endo[:R_t]]       = 1.

    # Spread return on capital - riskfree rate (flexible prices)
    Γ0[eq[:eq_spr_f], endo[:spr_f_t]]    = 1.
    Γ0[eq[:eq_spr_f], endo[:ERktil_f_t]] = -1.
    Γ0[eq[:eq_spr_f], endo[:r_f_t]]      = 1.

    # Entrepreneurs' leverage
    Γ0[eq[:eq_lev], endo[:lev_t]]  = 1.
    Γ0[eq[:eq_lev], endo[:qk_t]]   = -1.
    Γ0[eq[:eq_lev], endo[:kbar_t]] = -1.
    Γ0[eq[:eq_lev], endo[:n_t]]    = 1.

    # Convenience yield
    Γ0[eq[:eq_cy], endo[:cy_t]]     = 1.
    Γ0[eq[:eq_cy], endo[:b_liq_t]]  = -1.
    Γ0[eq[:eq_cy], endo[:b_safe_t]] = -1.

    # Ex ante real rate
    Γ0[eq[:eq_realr], endo[:rr_t]] = 1.
    Γ0[eq[:eq_realr], endo[:R_t]]  = -1.
    Γ0[eq[:eq_realr], endo[:Eπ_t]] = 1.

    # Credit
    Γ0[eq[:eq_cre], endo[:cre_t]]  = 1.
    Γ0[eq[:eq_cre], endo[:n_t]]    = m[:nstar]/(m[:kbarstar]-m[:nstar])
    Γ0[eq[:eq_cre], endo[:qk_t]]   = -m[:kbarstar]/(m[:kbarstar]-m[:nstar])
    Γ0[eq[:eq_cre], endo[:kbar_t]] = -m[:kbarstar]/(m[:kbarstar]-m[:nstar])

    # Nominal Deposit rate
    Γ0[eq[:eq_dep], endo[:Rd_t]]   = 1.
    Γ0[eq[:eq_dep], endo[:R_t]]  = -1.
    Γ0[eq[:eq_dep], endo[:b_liq_t]]   = (m[:σ_c]*(1 + m[:h]*exp(-m[:z_star])))/(1 - m[:h]*exp(-m[:z_star]))
    Γ0[eq[:eq_dep], endo[:b_safe_t]]  = (m[:σ_c]*(1 + m[:h]*exp(-m[:z_star])))/(1 - m[:h]*exp(-m[:z_star]))
