function init_pseudo_observable_mappings!(m::myModel1010depo_err)
...
# estimated with measurement error with their observed counterparts
	...
	pseudo[:AAAspread].name 		 = "Aaa spread"
	pseudo[:AAAspread].longname 	 = "Aaa - 20-year Treasury Spread"
	pseudo[:AAAspread].rev_transform = quartertoannual
	...
	pseudo[:LongRealRate].name 	 	    = "10y real yield"
	pseudo[:LongRealRate].longname 		= "10-year Real Bond Yield"
	pseudo[:LongRealRate].rev_transform = quartertoannual

	pseudo[:LongInflation].name 	 	    = "..."
	pseudo[:LongInflation].longname 		= "..."
	pseudo[:LongInflation].rev_transform    = loggrowthtopct_annualized
################################################################################
	...
	pseudo[:NominalRate].name = "Nominal Riks-Free Rate"
	pseudo[:NominalRate].longname = "Nominal Riks-Free Rate"
	pseudo[:NominalRate].rev_transform = quartertoannual

	pseudo[:RealDepositRate].name = "Ex Ante Real Deposit Rate"
	pseudo[:RealDepositRate].longname = "Ex Ante Real Interest Rate on Mutual Funds' Deposits"
	pseudo[:RealDepositRate].rev_transform = quartertoannual

	pseudo[:NominalDepRate].name = "Nominal Deposit Rate"
	pseudo[:NominalDepRate].longname = "Nominal Interest Rate on Deposits"
	pseudo[:NominalDepRate].rev_transform = quartertoannual
