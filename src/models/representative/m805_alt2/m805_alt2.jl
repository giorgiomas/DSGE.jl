"""
Model805_alt2{T} <: AbstractDSGEModel{T}

The Model805_alt2 type defines the structure of the Smets & Wouters (2007) model augmented
...
"""
mutable struct Model805_alt2{T} <: AbstractDSGEModel{T}
...
description(m::Model805_alt2) = "Smets-Wouters model, with long run inflation expectations"

"""
`init_model_indices!(m::Model805_alt2)`

Arguments:
`m:: Model805_alt2`: a model object

Description:
Initializes indices for all of `m`'s states, shocks, and equilibrium conditions.
"""
function init_model_indices!(m::Model805_alt2)
    # Endogenous states
    endogenous_states = [[
...
function Model805_alt2(subspec::String="ss20";
...
"""
```
init_parameters!(m::Model805_alt2)
```
...
those).
"""
function init_parameters!(m::Model805_alt2)
...
function model_settings!(m::Model805_alt2)
...
"""
```
parameter_groupings(m::Model805_alt2)
```
...
`prior_table`.
"""
function parameter_groupings(m::Model805_alt2)
    ...
    all_keys     = Vector[steadystate, sticky, financial, policy, processes, error]
    all_params   = map(keys -> [m[θ]::Parameter for θ in keys], all_keys)
    descriptions = ["Steady State", "Financial Frictions", "Nominal Rigidities", "Policy",
                    "Exogenous Processes",
                    "Measurement"]
